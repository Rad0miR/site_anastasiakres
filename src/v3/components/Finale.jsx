...
export default function Finale() {
  const { title, subtitle, notes } = useText().finale
  ...
  return (
    <motion.section
      ...
      {...revealOnScroll}
      className="relative isolate w-full overflow-hidden bg-v3-wine-deep"
    >
      ...
      <div className="relative flex min-h-[300px] w-full items-center justify-center px-4 py-14 [aspect-ratio:4/3] sm:px-5 sm:min-h-[300px] sm:[aspect-ratio:16/7] lg:[aspect-ratio:2135/736]">
        ...
        >
          <h2
            id="finale-title"
            className="font-v3-display text-[clamp(23px,2.6vw,48px)] font-light uppercase leading-[1.08] tracking-[0.12em] text-[#2A1E1C]"
          >
            {title.map((row) => (
          ...
          <p className="mt-3 text-[clamp(9px,1vw,15px)] font-light uppercase tracking-v3-widest text-[#4A3128] sm:mt-4">
            {subtitle}
          </p>
        </motion.div>
